...
const { baseUrl } = apiKeys.weatherBit;
const { weatherApiKey } = apiKeys.weatherBit;

const firebaseUrl = apiKeys.firebaseConfig.databaseURL;

// What the path should look like
// https://api.weatherbit.io/v2.0/forecast/daily?city=Raleigh,NC&units=i&key=API_KEY
const getForecast = (city, state) =>
	new Promise((resolve, reject) => {
		axios
			.get(`${baseUrl}?city=${city},${state}&units=i&key=${weatherApiKey}`)
			.then((result) => {
				if (result === '') {
					resolve('noData');
				} else {
...
const postRequest = (weather) =>
	axios.post(`${firebaseUrl}/weather.json`, weather);
