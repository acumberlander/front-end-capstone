...
class NewAppointmentForm extends React.Component {
	...
	postToFirebase = (appointmentDate) => {
		...
		weatherRequest.getForecast(city_par, state_par).then((forecast16) => {
	...
	formSubmit = (e) => {
		...
		if (!address.match(addressInput)) {
			alert(
				`Address can only include letters, numbers, and the period symbol(.). Please try again.`
			);
			return;
		}
			...
			<div className="newAppointmentContainer container-fluid p-0">
				...
				<div id="quoteColumn" className="col-3 p-0">
					...
					<div className="nextButtonDiv">
						<button onClick={this.formSubmit} className="nextButton">
							Next
						</button>
					</div>
				</div>
